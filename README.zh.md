...
NoBrainFog 是一个 AI 驱动的个人任务入口系统。它把你在 Discord、企业微信、命令行或 Email 里的碎片化想法整理成统一的 Markdown `todo.md`。

核心思想：聊天平台、命令行和邮箱都只是入口，真正的数据层是同一个 `todo.md`。
...
- Discord / 企业微信 / Email 共用同一套核心逻辑。
- 每个 adapter 使用独立私有配置文件。
- 所有任务写入同一个 `todo.md`。
- 支持导出 Markdown，也支持格式化 Excel `.xlsx`。
- 支持 Email 作为远程任务捕捉、报告和导出入口。
...
/root/nobrainfog-config/
  ...
  email.env                        # Email IMAP 私有配置，不进 Git
...
cp /root/NoBrainFog/email.env.example /root/nobrainfog-config/email.env
...
不要提交真实 env 文件。它们包含 Discord token、企业微信 Secret、Email App Password、AI API Key 等敏感信息。
...
Discord 是最完整的日常管理入口，适合做 `/undo`、`/done`、`/edit`、`/pri`、`/due` 等高频操作。

## Email IMAP 配置

Email adapter 会把一个工具人邮箱变成 NoBrainFog 的远程入口。
...
```text
/ping                    回一封健康检查邮件
/help 或 /h              回 Email 命令说明
/report 或 /r 或 /rep    回当前任务报告
/export 或 /e 或 /exp    回 todo.md 附件
/excel 或 /xlsx          回格式化 Excel 附件
```
...
Gmail 要用 Google App Password，不要用普通登录密码。App Password 中间的空格会被 NoBrainFog 自动去掉。

白名单非常重要，因为 Email 命令可以导出你的任务库：
...
SMTP 回信默认使用 Gmail SMTP。如果 `EMAIL_SMTP_USERNAME` 和 `EMAIL_SMTP_PASSWORD` 留空，会复用 `EMAIL_USERNAME` 和 `EMAIL_PASSWORD`：
...
建议用 tmux 脚本管理 Email adapter：

```bash
~/bots.sh check nbf-embot
~/bots.sh restart nbf-embot
~/bots.sh logs nbf-embot
```
...
企业微信当前作为 secondary adapter 维护。Discord 和 Email 更适合作为日常主力入口。
...
/ping                       回健康检查邮件
/help 或 /h                 回 Email 命令说明
/report 或 /r 或 /rep       回任务报告邮件
/export 或 /e 或 /exp       回 todo.md 附件
/excel 或 /xlsx             回 Excel 附件
```

不同 adapter 的能力不是完全相同。Discord 是最完整的控制台，Email 重点负责捕捉、报告和导出。
...
推荐用你的 `~/bots.sh` tmux 管理脚本，例如：

```text
nbf-dcbot  → Discord adapter
nbf-wcbot  → WeChat Work adapter
nbf-embot  → Email IMAP adapter
```
...
~/bots.sh status
~/bots.sh check nbf-embot
~/bots.sh restart nbf-dcbot
~/bots.sh restart nbf-wcbot
~/bots.sh restart nbf-embot
~/bots.sh logs nbf-embot
~/bots.sh attach nbf-embot
```
...
Email adapter 写 /root/nbf-vault/todo.md
...
1. 邮件是否是未读。
2. 发件人是否在 `EMAIL_ALLOWED_SENDERS` 或 `EMAIL_ALLOWED_DOMAINS` 中。
3. `~/bots.sh logs nbf-embot` 是否显示 `Sender whitelist: enabled`。
4. Gmail App Password 是否仍然有效。

### Email 命令没有被识别

Email command parser 优先看正文第一行，正文为空才看 subject。
...
如果 subject 写 `/export`，正文必须为空，否则正文第一行会优先。

### Email 可以收但不能回信

检查：

1. `EMAIL_SMTP_ENABLED=true`。
2. Gmail App Password 是否可用于 SMTP。
3. `EMAIL_SMTP_HOST=smtp.gmail.com`。
4. `EMAIL_SMTP_PORT=587`。
5. 如果 SMTP 用户名/密码留空，会复用 `EMAIL_USERNAME` 和 `EMAIL_PASSWORD`。
...
- 不要提交真实 env 文件。
- 不要把 Discord token、企业微信 Secret、Email App Password、AI API Key 发到聊天里。
- Email 命令可以导出任务库，建议 `EMAIL_ALLOWED_SENDERS` 使用精确邮箱。
- 企业微信生产环境使用 HTTPS。
- 定期轮换 Discord token、企业微信 Secret、Email App Password 和 AI API Key。
...
adapters/email_imap.py          # Email IMAP 轮询 adapter
core/help_text.py               # Discord / WeChat 帮助文案
core/email_commands.py          # Email 命令和别名
core/email_parser.py            # Email 正文清洗
core/email_sender.py            # SMTP 回信和附件发送
core/email_dedupe.py            # Email 去重 marker
